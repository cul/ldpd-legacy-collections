...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <xsl:for-each select="//marc:datafield[@tag = '035']/marc:subfield[@code = 'a']">
            <xsl:if test="starts-with(.,'(CStRLIN)')">
                ...
                <xsl:result-document
                    href="{$resultPath}{$collection_id}/ldpd_ferriss_{translate($item_id, '.', '_')}_mods.xml">
                ...
                <xsl:if test="count(//marc:datafield[@tag = '789']) > 1">
                    <xsl:result-document
                        href="structMap/{$collection_id}/ldpd_ferriss_{translate($item_id, '.', '_')}_structMap.xml">
    ...
    <xsl:template name="structMap">
        ...
        <mets:structMap xmlns:mets="http://www.loc.gov/METS/" LABEL="ldpd.ferriss.{$item_id}">
            <xsl:attribute name="LABEL"><xsl:text>ldpd.ferriss.</xsl:text><xsl:value-of select="$item_id"/></xsl:attribute>
            <xsl:for-each select="//marc:datafield[@tag = '789']">
                <xsl:variable name="part_id">
                    <xsl:value-of select="child::marc:subfield[@code = 'i']"/>
                </xsl:variable>
