...
java -cp $SAXON_PATH net.sf.saxon.Transform -s:marc/ferriss -o:. -xsl:xslt/aviador2mods.xsl
for f in `find mods/ferriss -name "*.xml"`; do
  java -cp $SAXON_PATH net.sf.saxon.Transform -s:$f -xsl:xslt/tidy.xsl -o:$f
done
